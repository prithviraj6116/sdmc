...
class IGMPv3Router(EventMixin):
    ...
    def listen_on_connection(self, connection):
        ...
        frameinfo = getframeinfo(currentframe());log.info('PPP %s:%s:%s' % (msg, frameinfo.filename, frameinfo.lineno))
...
class IGMPManager(EventMixin):
    ...
    def __init__(self):
        frameinfo = getframeinfo(currentframe());log.info('PPP %s:%s' % (frameinfo.filename, frameinfo.lineno))
    ...
    def add_igmp_router(self, router_dpid, connection):
        ...
        if not router_dpid in self.routers:
            ...
            frameinfo = getframeinfo(currentframe());log.info('PPP %s:%s' % (frameinfo.filename, frameinfo.lineno))
            log.info('dpid PPPPP %s:%s' % (router.dpid, connection.dpid));
    ...
    def _handle_LinkEvent(self, event):
        ...
        log.info('Got LinkEvent: ' + dpid_to_str(event.link.dpid1) + ' --> ' + dpid_to_str(event.link.dpid2) + ' LinkUp: ' + str(not event.removed))
        log.info('Got LinkEvent: ' + str(event.link) + ' : '+ str(event.link[1]) + ' --> ' + str(event.link[2]) + ':' + str(event.link[3]) + ':' + str(event.link[0]) + ' LinkUp: ' + str(not event.removed))
    ...
    def _handle_PacketIn(self, event):
        ...
        if not igmp_pkt is None:
            ...
            log.info(str(receiving_router) + ':' + str(event.port) + '| Received IGMP packetPPP')
