...
sudo ~/pox/pox.py $debug  openflow.of_01 --port=10001 forwarding.l2_learning &> pox10001.log &
sudo ~/pox/pox.py  $debug openflow.of_01 --port=10002 forwarding.l2_learning &> pox10002.log &
...
fvctl -f /dev/null add-slice video tcp:localhost:10001 admin1@upperslice;
fvctl -f /dev/null add-slice non-video tcp:localhost:10002 admin2@upperslice;
