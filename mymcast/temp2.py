...
def main():
    ...
    while True:
        try:
            ...
            print 'packet received' + str(pickle.loads(data)) + ' from ' + str(addr[0])
            #echo_socket.sendto(data, (addr[0], echo_port))
            # print 'packet received' + str(pickle.loads(data)) + ' to ' + str(addr[0]) + ':' + str(echo_port)
            #recv_packets += 1
